...
BEGIN
	-- Überprüfe wieder, ob der Benutzer dessen Passwort geändert werden soll existiert
	SELECT sql_exist_User(p_Name) INTO @exist;
	IF @exist = 1 THEN
		-- Falls er existiert, führe das Passwort-Update der User-Tabelle durch
		UPDATE user SET Passwort=p_newPassword WHERE Email = p_Name;
		SELECT Passwort from user  WHERE Email = p_Name into @passwd;
		-- Nun wird überprüft ob das neue Passwort tatsächlich gesetzt wurde
