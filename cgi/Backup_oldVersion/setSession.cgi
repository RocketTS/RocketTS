...
print $session->param('input_Site');
...
#  when( undef )							 { Login::html_login(); } 
  ...
  when('Registration_init')	 			 { $session->param('input_Site',"Registration_init");
  											print $cgi->meta({-http_equiv => 'REFRESH', -content => '0; /cgi-bin/rocket/Dummy.cgi'}); }
  
  when('Registration_check')	 	 	{ $session->param('input_Site',"Registration_check");
  											print $cgi->meta({-http_equiv => 'REFRESH', -content => '0; /cgi-bin/rocket/Dummy.cgi'}); }
