...
use myGraph;
 ...
 sub print_show_newTickets {
 	...
 	#gibt alle neuen Tickets aus, die der Mitarbeiter laut seinen Rechten und F‰higkeiten sehen kann
 	#R¸ckgabe: Ausgabe auf der Webseite
 ...
 sub print_show_specTicket {
	...
	my $session = CGI::Session->new($cgi);
	#Bezieht die ausgew‰hlte TicketID aus der Session
 ...
 sub print_submit_assumeTicket {  
 	...
 	#wird ausgef¸hrt, wenn Mitarbeiter das angegebenes Ticket ¸bernimmt (Button-geklickt)
 	#R¸ckgabe: Ausgabe der Statusmeldung auf der Webseite, ob ‹bernahme erfolgreich

	my $cgi = new CGI;
 	my($Username,$Ticket_ID) = @_;
 	my $success = MitarbeiterDB::assume_Ticket($Username,$Ticket_ID); # boolscher Wert, ob ‹bernahme erfolgreich
 	if($success != 0)
 	{
  ...
  sub print_submit_forwardTicket {
  	...
  	#R¸ckgabe: Ausgabe der Statusmeldung, ob weiterleiten erfolgreich
  ...
  sub print_submit_releaseTicket { 
  	...
  	#R¸ckgabe: Ausgabe der Statusmeldung, ob Freigabe erfolgreich
  ...
  sub print_submit_closeTicket { 
  	...
  	#R¸ckgabe: Anzeige der Statusmeldung, ob schlieﬂen erfolgreich

	my $cgi = new CGI;
 	my($Username,$Ticket_ID) = @_;
 	my $success = MitarbeiterDB::close_Ticket($Username,$Ticket_ID); #boolscher Wert, ob schlieﬂen des Tickets erfolgreich
...
sub print_Statistik{
	...
	#R¸ckgabe: Ausgabe der Graphen auf der Webseite
 	my $cgi = new CGI;
	my $session = CGI::Session->new($cgi);
